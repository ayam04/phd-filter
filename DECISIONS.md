# Data Quality — Challenges & Defenses

Surfacing the *right humans* is the core difficulty in supervisor matching. This document lists the
real failure modes the system defends against, with **concrete examples from the committed
`sample_output/106419.json`** (a synthesized profile: computational psychiatry / PTSD + stress
neurobiology + psychiatric/single-cell genomics; target US/UK/Australia; Indian nationality).

Per-stage funnel for that run:
...
Governing principle: **precision over recall.** Contamination (a wrong pick) is far more damaging
than a missed borderline-correct one, and country adherence is a hard requirement — so every stage
prefers dropping a borderline match to admitting a wrong one.

---

## 1. Wrong-domain leakage from keyword overlap — primary defense
...
**Concrete evidence.** In this run the gate rejected **81 of 240** candidates as domain mismatches.
Four canonical keyword-collision cases are encoded as always-on regression tests
...
**Trade-off.** `domain_match` is deliberately set to "same broad field or directly adjacent subfield
a domain expert would endorse," not "exact area only." Tightening it to exact-only cut the pass rate
from ~67% to ~33% and discarded clearly-relevant adjacent PIs (depression/anxiety genomics for a
PTSD-genomics student). The four cross-discipline traps still fail because they are different
*disciplines*, not adjacent subfields.

## 2. Career-stage errors — surfacing non-PIs
...
- **Fellowship-awardee trap:** `grants.py` flags personal-award activity codes
  (NIH F30/F31/F32/F33/K99/R00/K-series/T32) and UKRI Studentship/Fellowship categories, so the
  listed person is treated as a junior awardee, not a PI (`fellowship_awardee()` + regression test).
...
surfaced during the self-audit (§7): **Mette A. Peters** is a research-data-science *director* at
Sage Bionetworks (she coordinates data portals; her listed email is `…@nih.gov`), not a lab-running
PI — she appears on many consortium papers and initially slipped through, so the gate was strengthened
to reject "research-data-coordinator / core-facility / consortium staff," and she is now correctly
excluded. The regression suite confirms a 2-years, 3-paper, first-author-only profile is rejected
while a 20-year, 80-work, senior-author profile is accepted, and that an F32-coded grant flags its
awardee as junior. (The cheap filter dropped 0 here because the top-400-by-appearance candidates are
already prolific; the LLM gate is the binding non-PI check for this profile — both layers exist so one
covers the other's blind spots.)

## 3. Same-name-different-person collisions

**Approach.** The system never resolves a person from a name string. Every candidate is an **OpenAlex
Author ID** (a stable, disambiguated identity, e.g. `A5031152245`). On top of that, the verification
gate is asked whether the abstracts "look like different people merged under one name"
(`is_collision`), and a `WRONG_PERSON` outcome in the feedback loop flows back into suppression.

**Concrete evidence.** Output `supervisor_id`s are OpenAlex Author IDs. Because the *aggregate* of an
author's matched works must cohere to the area (not a single paper title), a paper that merely shares
a title keyword cannot by itself surface a person. A final same-name de-duplication also removes the
occasional OpenAlex same-author ID split (which had surfaced a duplicate "Mette A. Peters").

## 4. Country adherence + institution quality
...
OpenAlex data-quality artifacts and out-of-country leaks that the self-audit (§7) caught:
- **Daniëlle Posthuma** and **Ole A. Andreassen** — world-class psychiatric geneticists, but their
  ...
  are now **dropped** as out-of-country, instead of being recommended to a student who cannot study
  in NL/Norway.
...
This matters twice: a wrong country is disqualifying, and "Naomi Wray, Pioneer (United States)" reads
as an embarrassing mismatch even when the country happens to be right.

## 5. Eligibility filters in free-text ads
...
## 6. Evidence integrity + no fabricated contact details
...
## 7. Adversarial self-audit (how this was validated, and what it changed)

The gate is not trusted blindly. A **domain-expert-style audit** re-verifies the top 30 picks against
the web through independent reviewers (is this a real senior PI? at the claimed institution/country?
genuinely in-domain?).

**First pass: 26/30 (87%) strong matches, 5 contaminations.** It caught real problems the automated
pipeline missed at the time:
- **Out-of-country leaks** — Daniëlle Posthuma (**Netherlands**) and Ole Andreassen (**Norway**)
...
- **Mislabeled institutions** — Daniel Smith shown at "Augsburg University (US)" vs his real Edinburgh (UK).

**Fixes applied** (root-cause, not patches): the current-primary-academic-affiliation country gate,
a strengthened non-PI criterion, and a same-name de-duplication.

**Second pass on the corrected output: 28/30 (93%), 2 contaminations** — and both are only
...
genomics, but his primary role is now an industry VP with an *adjunct* university title). The system
stops here deliberately: tightening further to remove two debatable stretches risks dropping
legitimate adjacent-field PIs. Those two residuals are an honest known limit, not a solved problem.
...
`--adjustments adjustments.json` measurably changes the ranking
(`sample_output/106419_after_feedback.json`):
...
Caveats (the data is sparse and biased): outcomes exist only for *emailed* PIs (survivorship bias), so
unseen PIs keep their base score (cold-start), and priors are smoothed heavily (Laplace) so a single
outcome cannot dominate. `WRONG_PERSON` additionally feeds the disambiguation signal, not just ranking.

---

## Bounded trade-offs (noticed, consciously limited)

- **OpenAlex humanities/regional coverage** is weaker than STEM/medicine — fine for this profile, flagged for humanities students.
- **Grant→PI fuzzy linking across national databases** is intentionally *not* done; the system uses OpenAlex grants attached to a PI's own papers (zero linking risk). Aggressive fuzzy linking would add the very contamination the design is built to avoid.
- **Live vacancy scraping** is reduced to the eligibility module; supervisors are the unit, positions are enrichment.
- **The embedding pre-filter barely binds** (dropped 2/400) because `bge-small` is not very discriminative on dense biomedical abstracts; the LLM gate is the real domain filter. The embedding stage is kept as a cheap ordering signal, not relied on for rejection.
